...
test('the build catalog grows by twelve decorations without disturbing existing kinds',()=>{
  ...
  assert.equal(Object.keys(BUILDINGS).length,86,'85 before + the gate that opens a wall line');
  assert.equal(Object.keys(BUILDINGS).filter(kind=>BUILDINGS[kind as keyof typeof BUILDINGS].category==='decoration').length,26,'nine trees and garden pieces plus twelve street decorations plus three ornaments plus the wall and its gate');
