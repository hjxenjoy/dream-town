...
export class GameUI {
  ...
  private handle(action:string,value?:string){
    ...
    switch(action){
      ...
      case 'journey-tab':this.journeyTab=value as 'growth'|'achievements'|'styles'|'projects'|'quests';this.renderPanel();return;
  ...
  update(){
    ...
    this.html(this.root.querySelector('#calendar')!,`<span class="season-icon">${icon(s.season==='winter'?'spark':s.season==='autumn'?'leaf':'sun',29)}</span><span><b>${SEASON_NAMES[s.season]} <em>第 ${clock.day} 天 ${clock.label}</em></b><small>${clock.partName} · ${clock.note}</small></span>`);
