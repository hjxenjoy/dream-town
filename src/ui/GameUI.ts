...
export class GameUI {
  ...
  renderPanel(){
    ...
    if(this.panel==='farming'){content=farmingContent(this.world,this.farmId,this.gardenAlbum);
      ...
      const blocked=b.damaged?'需要修复':b.paused?'已暂停':def.workers&&b.workers===0?'缺少工人':production?.blocked==='materials'?'原料不足':production?.blocked==='warehouse'?'仓位不足':production?.blocked==='target'?'库存充足':production?.blocked==='reserve'?'保留建造木材':null;
