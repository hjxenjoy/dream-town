...
import { valleyMap, districtButtons, presetMap, type MapDestination } from './valleyMap';
import { MAP_PRESETS, MAP_PRESET_IDS, DEFAULT_MAP_PRESET, presetCensus, presetRoads, type MapPresetId } from '../sim/presets';
...
type Panel='farming'|'layout'|'build'|'warehouse'|'orders'|'caravan'|'residents'|'quests'|'settings'|'building'|'help'|'mayor'|'technology'|'supply'|'map'|'roads'|'presets'|null;
export interface UICallbacks { action: (fn:()=>ActionResult,id?:string,effect?:string)=>void; build:(kind:BuildingKind|null)=>void; road:(kind:RoadKind|'remove'|null)=>void; focus:(id:string)=>void; zoom:(delta:number)=>void; home:()=>void; district:(id:MapDestination)=>void; move:(id:string)=>void; speed:(value:number)=>void; sound:()=>void; save:(slot?:string)=>Promise<void>; load:(slot:string)=>Promise<void>; export:()=>void; import:(file:File)=>Promise<void>; fullscreen:()=>void; listSaves:()=>Promise<{slot:string;savedAt:number;level:number;population:number}[]>; newGame:(id:MapPresetId)=>Promise<void>; }
export class GameUI {
  ...
  private handle(action:string,value?:string){
    ...
    switch(action){
      ...
      case 'preset-new':void this.cb.newGame(value as MapPresetId);break;
  ...
  renderPanel(){
    ...
    } else if(this.panel==='presets'){content=this.presetsContent();
  ...
  /**
   * The map picker: each blueprint drawn as its own plan, so choosing a starting town is
   * looking at streets, blocks and yards rather than reading a feature list. Applying one
   * replaces the current town — the confirm for that lives in the callback, beside the save.
   */
  private presetsContent(){
    return `<p class="panel-intro">每一张都是规划好的开局蓝图：街区、道路、工坊与装饰已经各就各位，居民一开局就在街上走。</p><div class="preset-grid">${MAP_PRESET_IDS.map(id=>{
      const map=MAP_PRESETS[id],census=presetCensus(map);
      return `<article class="preset-card ${id===DEFAULT_MAP_PRESET?'featured':''}">
        <div class="preset-preview">${presetMap(map)}</div>
        <h3>${esc(map.name)} <small>${esc(map.tagline)}</small></h3>
        <p>${esc(map.description)}</p>
        <div class="preset-facts"><span>${icon('home',15)} 建筑 ${map.placements.length}</span><span>${icon('road',15)} 道路 ${presetRoads(map).length} 格</span><span>${icon('people',15)} 居民 ${map.population}</span><span>${icon('wheat',15)} 床位 ${census.beds}</span></div>
        ${this.button('preset-new',id,`${icon('hammer',17)} 用「${esc(map.name)}」重新开局`,false,'game-button wide')}
      </article>`;
    }).join('')}</div><p class="muted">重新开局会替换当前小镇。想留住现在的进度，先在「设置」里存入手动存档或导出备份。</p>`;
  }
  private roadContent(){
