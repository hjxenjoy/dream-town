...
export interface BuildingDefinition {
  ...
  workers?: number;
  /**
   * How much ground this kind claims: a square footprint of this many tiles, anchored at the
   * building's north-west corner. Small props stand on one tile, houses and workshops on four,
   * and the landmarks — the hall, the castle, the theatre, the quay — on nine, which is their
   * yard rather than a bigger sprite. A save from before footprints carries none on the
   * building itself and keeps the compact spacing it was built with.
   */
  footprint: 1 | 2 | 3;
  environment?: number;
  housing?: number;
...
export const BUILDINGS: Record<BuildingKind, BuildingDefinition> = {
  flowernursery:{footprint: 2, name:'四季花圃',description:'自动培育并收好花束，为花色养成留下一份积累。',category:'production',cost:360,wood:12,stone:4,frame:null,cycle:65,output:{flowers:3},workers:1,environment:4,autoCollect:true},
  orchardhouse:{footprint: 2, name:'果园小屋',description:'照看一片小果园，自动采收鲜果，供果酱坊加工。',category:'production',cost:480,wood:16,stone:6,frame:null,cycle:75,output:{fruit:5},workers:1,autoCollect:true},
  chickencoop:{footprint: 2, name:'咯咯鸡舍',description:'自动用一袋饲料照顾鸡群，收好三枚鸡蛋；没有饲料时歇一歇。',category:'production',cost:300,wood:12,stone:3,frame:null,cycle:45,input:{feed:1},output:{eggs:3},workers:1,autoCollect:true},
  jamkitchen:{footprint: 2, name:'甜果果酱坊',description:'自动将三份鲜果熬成两罐果酱。果酱为邻居添一份闲适，也可出售。',category:'production',cost:620,wood:18,stone:12,frame:null,cycle:55,input:{fruit:3},output:{jam:2},workers:1,autoCollect:true},
  teahouse:{footprint: 2, name:'河畔茶屋',description:'邻居歇脚聊天的地方，每级提供六个社区名额与十点服务。',category:'services',cost:720,wood:18,stone:10,frame:null,populationCap:6,services:10,environment:5},
  homestead:{footprint: 2, name:'花窗农庄',description:'带庭院、花窗与晾衣角的农庄住宅，每级提供十个床位。',category:'homes',cost:680,wood:20,stone:12,frame:null,housing:10,environment:5},
  mine: {footprint: 2,  name: '赤岩矿场', description: '循着山石里的赤色纹路，每轮开采 5 份矿石。冶炼产业从这里起步。', category: 'production', cost: 520, wood: 14, stone: 12, frame: 0, technology: 'mining', cycle: 38, output: { ore: 5 }, workers: 2 },
  kiln: {footprint: 2,  name: '松烟炭窑', description: '将 2 份木材烧成 2 份木炭，自动保留建造用木材，为炉火准备燃料。', category: 'production', cost: 380, wood: 12, stone: 10, frame: 1, technology: 'mining', cycle: 32, input: { wood: 2 }, output: { charcoal: 2 }, workers: 1, preserveWood: true },
  smelter: {footprint: 2,  name: '炉光冶炼坊', description: '用 5 份矿石与 2 份木炭冶炼 1 块金属锭，点亮小镇的工业梦想。', category: 'production', cost: 860, wood: 18, stone: 24, materials: { materials: 3 }, frame: 2, technology: 'metallurgy', cycle: 60, input: { ore: 5, charcoal: 2 }, output: { ingot: 1 }, workers: 2 },
  smithy: {footprint: 2,  name: '叮当铁匠铺', description: '将 1 块金属锭和 2 块木板打造成 2 套工具。工具可出口，也用于研究精工技术。', category: 'production', cost: 920, wood: 20, stone: 18, materials: { materials: 3 }, frame: 3, technology: 'metallurgy', cycle: 48, input: { ingot: 1, plank: 2 }, output: { tools: 2 }, workers: 2 },
  feedmill: {footprint: 2,  name: '谷穗饲料坊', description: '将 3 份小麦配成 4 袋饲料，给牧场的小羊准备一顿饱饭。', category: 'production', cost: 340, wood: 12, stone: 6, frame: 4, technology: 'husbandry', cycle: 28, input: { wheat: 3 }, output: { feed: 4 }, workers: 1 },
  pasture: {footprint: 2,  name: '绵绵牧场', description: '先用 2 袋口粮照料小羊 120 秒，成年后每轮用 2 袋饲料产出 3 团羊毛，自动收好。', category: 'production', cost: 460, wood: 18, stone: 6, frame: 5, technology: 'husbandry', cycle: 40, input: { feed: 2 }, output: { wool: 3 }, workers: 1, autoCollect: true },
  weaver: {footprint: 2,  name: '蓝梭织布坊', description: '把 3 团羊毛织成 2 卷布料，接着交给裁缝，或用于改良仓储。', category: 'production', cost: 640, wood: 20, stone: 12, materials: { materials: 2 }, frame: 6, technology: 'tailoring', cycle: 42, input: { wool: 3 }, output: { cloth: 2 }, workers: 2 },
  tailor: {footprint: 2,  name: '暖衣裁缝铺', description: '用 2 卷布料缝制 2 件衣物，让小镇的手艺走进邻居的生活。', category: 'production', cost: 760, wood: 18, stone: 14, materials: { materials: 2 }, frame: 7, technology: 'tailoring', cycle: 48, input: { cloth: 2 }, output: { clothing: 2 }, workers: 2 },
  farmhouse: {footprint: 2,  name: '田园小院', description: '带小庭院的乡间住宅，每级提供 8 个床位与 3 点环境值。适合低密度的田园街区。', category: 'homes', cost: 460, wood: 14, stone: 8, frame: null, housing: 8, environment: 3 },
  rowhouse: {footprint: 2,  name: '彩窗联排屋', description: '两家相邻，共享一条花巷。每级提供 12 个床位，适合沿街成排布置。', category: 'homes', cost: 820, wood: 18, stone: 18, materials: { materials: 3 }, frame: null, housing: 12 },
  apartment: {footprint: 2,  name: '晴空公寓', description: '有花阳台的三层住宅，每级提供 18 个床位。记得配齐社区人口名额与饮水。', category: 'homes', cost: 1480, wood: 20, stone: 30, materials: { materials: 8 }, frame: null, housing: 18 },
  forester: {footprint: 2,  name: '青林伐木屋', description: '专注供应原木，每轮采集 10 份木材。库存够用自动休工，为建造与取暖储备木料。', category: 'production', cost: 540, wood: 10, stone: 8, frame: null, cycle: 26, output: { wood: 10 }, workers: 2 },
  sawmill: {footprint: 2,  name: '河谷锯木厂', description: '将 4 份木材加工成 6 块木板。自动保留建造与取暖所需木材，适合商队和铁匠铺集中供货。', category: 'production', cost: 680, wood: 16, stone: 12, frame: null, technology: 'mining', cycle: 36, input: { wood: 4 }, output: { plank: 6 }, workers: 1, preserveWood: true },
  fishpond: {footprint: 2,  name: '碧水养鱼场', description: '用 3 袋饲料养出 8 条鲜鱼，四季稳定供应口粮，让麦田也能养活更多邻居。', category: 'production', cost: 580, wood: 14, stone: 10, frame: null, technology: 'husbandry', cycle: 40, input: { feed: 3 }, output: { fish: 8 }, workers: 1 },
  brickworks: {footprint: 2,  name: '窑火建材坊', description: '将 6 份石料和 1 份木炭制成 1 包建材，补充商队带回的物资，支持住宅与市政扩建。', category: 'production', cost: 980, wood: 14, stone: 24, frame: null, technology: 'mining', cycle: 65, input: { stone: 6, charcoal: 1 }, output: { materials: 1 }, workers: 2 },
  school: {footprint: 2,  name: '晨读小学', description: '铃声响起，孩子们有了课堂。每级增加 12 位社区人口名额与 18 点社区服务。', category: 'services', cost: 1100, wood: 20, stone: 18, materials: { materials: 5 }, frame: null, populationCap: 12, services: 18 },
  clinic: {footprint: 2,  name: '安心诊所', description: '照顾邻居的日常健康。每级让 10 位居民康健，并增加 10 位社区人口名额与 25 点社区服务。', category: 'services', cost: 900, wood: 12, stone: 20, materials: { materials: 4 }, frame: null, populationCap: 10, services: 25, health: 10 },
  ...
  harbor: {footprint: 3,  name: '河湾码头', description: '有码头才能出海。建起它，商队便能扬帆去四座小岛，换回河谷种不出的特产。', category: 'services', cost: 2200, wood: 28, stone: 24, materials: { materials: 10 }, frame: null, minTownLevel: 10, services: 18 },
  zoogate: {footprint: 2,  name: '青岚动物园', description: '动物园的大门。建起它，镇外才有人来看动物；之后才能修展区与纪念品铺。', category: 'services', cost: 2400, wood: 30, stone: 30, materials: { materials: 10 }, frame: null, technology: 'husbandry', minTownLevel: 12, environment: 8, services: 20 },
  zooenclosure: {footprint: 2,  name: '动物展区', description: '一个展区住一种动物。它们每天吃饲料，来参观的人留下纪念品；在「展区住户」里换动物。', category: 'production', cost: 900, wood: 20, stone: 16, materials: { materials: 4 }, frame: null, technology: 'husbandry', cycle: 50, input: { feed: 2 }, output: { souvenir: 3 }, workers: 1, autoCollect: true, needsZooGate: true },
  zooshop: {footprint: 2,  name: '纪念品小铺', description: '门口的小铺把纪念品卖出好价钱——只要它开着，每份纪念品都更值钱。', category: 'services', cost: 700, wood: 14, stone: 10, frame: null, technology: 'husbandry', needsZooGate: true, services: 12, environment: 3 },
  tavern: {footprint: 2,  name: '晚风酒馆', description: '有酒的酒馆才热闹。啤酒会在这里被喝掉，让邻居们多几分好心情；镇上没有酒馆时，啤酒只是能卖的货。', category: 'services', cost: 760, wood: 18, stone: 12, materials: { materials: 2 }, frame: null, technology: 'viniculture', services: 22, populationCap: 4 },
  theatre: {footprint: 3,  name: '星幕小剧院', description: '灯光与戏剧丰富夜晚。每级增加 16 位社区人口名额、30 点社区服务与 5 点环境值。', category: 'services', cost: 1800, wood: 24, stone: 28, materials: { materials: 8 }, frame: null, technology: 'civics', populationCap: 16, services: 30, environment: 5 },
  chapel: {footprint: 2,  name: '河畔小教堂', description: '钟声安顿人心。每级为 14 户邻居带来信仰与安定的生活，并增加 8 位社区人口名额。', category: 'services', cost: 1500, wood: 22, stone: 26, materials: { materials: 6 }, frame: null, populationCap: 8, faith: 14 },
  guardpost: {footprint: 2,  name: '路口岗哨', description: '巡逻半径 5 格，把打家劫舍的强盗挡在外面。每级扩大 1 格。', category: 'services', cost: 350, wood: 12, stone: 8, frame: null, guardRadius: 5 },
  castle: {footprint: 3,  name: '青岚城堡', description: '石墙与塔楼的常备要塞，警戒半径 11 格，把强盗挡在整片城区之外。每级扩大 1 格。', category: 'services', cost: 2600, wood: 40, stone: 45, materials: { materials: 12 }, frame: null, technology: 'metallurgy', guardRadius: 11 },
  barracks: {footprint: 2,  name: '青岚兵营', description: '常驻守备队，警戒半径 8 格，挡住强盗的洗劫。每级扩大 1 格。', category: 'services', cost: 1500, wood: 28, stone: 30, materials: { materials: 8 }, frame: null, technology: 'metallurgy', guardRadius: 8 },
  wall: {footprint: 1,  name: '石砌城墙', description: '一段城墙，连成一片就成一道防线。相邻的墙会自动接上。', category: 'decoration', cost: 90, wood: 0, stone: 6, frame: null, environment: 1 },
  citygate: {footprint: 1,  name: '城门', description: '城墙上的门洞。城墙会挡住去路，围起来的街区要在墙上留一道门，居民与搬运工才进得来、出得去。', category: 'decoration', cost: 180, wood: 0, stone: 12, frame: null, environment: 1 },
  watertower: {footprint: 2,  name: '蓝顶供水塔', description: '为半径 7 格内的住宅供水，每次升级扩大 1 格，适合扩张中的住宅区。', category: 'services', cost: 780, wood: 10, stone: 24, materials: { materials: 3 }, frame: null, waterRadius: 7 },
  firestation: {footprint: 2,  name: '赤砖消防站', description: '守护半径 7 格内的建筑，每次升级扩大 1 格，让工业区也能安心生产。', category: 'services', cost: 1050, wood: 16, stone: 24, materials: { materials: 4 }, frame: null, technology: 'metallurgy', fireRadius: 7 },
  cottage: {footprint: 2,  name: '林间小屋', description: '温暖的新家，提供 6 个床位。居民幸福、粮食充足时会迎来新邻居。', category: 'homes', cost: 320, wood: 12, stone: 5, frame: 0, housing: 6 },
  windmill: {footprint: 2,  name: '风车磨坊', description: '风吹麦香。将 3 份小麦磨成 3 袋面粉，是面包产业的起点。', category: 'production', cost: 560, wood: 18, stone: 8, frame: 1, cycle: 28, input: { wheat: 3 }, output: { flour: 3 }, workers: 2 },
  bakery: {footprint: 2,  name: '晨光面包房', description: '用 3 袋面粉和 1 份砂糖烤出 4 个面包，为居民与订单准备早餐。', category: 'production', cost: 720, wood: 20, stone: 12, frame: 2, cycle: 38, input: { flour: 3, sugar: 1 }, output: { bread: 4 }, workers: 2 },
  townhall: {footprint: 3,  name: '小镇议事厅', description: '小镇的心脏。提升社区人口上限，承载居民共同的梦想。', category: 'services', cost: 1200, wood: 35, stone: 25, frame: 3, populationCap: 6, services: 35 },
  lumber: {footprint: 2,  name: '林间木工坊', description: '每轮采集 6 份木材、加工 2 块木板。可切换木材或木板优先，均衡模式会按库存补货。', category: 'production', cost: 380, wood: 10, stone: 5, frame: 4, cycle: 24, output: { wood: 6, plank: 2 }, workers: 2 },
  quarry: {footprint: 2,  name: '山麓采石场', description: '采集结实的山石，每轮提供 3 份石料，支持小镇扩建。', category: 'production', cost: 420, wood: 12, stone: 5, frame: 5, cycle: 32, output: { stone: 3 }, workers: 2 },
  fishery: {footprint: 2,  name: '河畔渔屋', description: '四季都有收获，每轮捕获 4 条鲜鱼，是可靠的日常食物来源。', category: 'production', cost: 280, wood: 10, stone: 3, frame: 6, cycle: 22, output: { fish: 4 }, workers: 1 },
  market: {footprint: 2,  name: '旅行者集市', description: '邻镇商人汇聚于此。订单与商队将小镇的收获变成扩建的机会。', category: 'services', cost: 640, wood: 18, stone: 12, frame: 7, services: 35 },
  well: {footprint: 2,  name: '清泉水井', description: '为半径 4 格内的居民提供清水，改善饮水需求。', category: 'services', cost: 180, wood: 5, stone: 10, frame: 8, waterRadius: 4 },
  warehouse: {footprint: 2,  name: '丰收仓库', description: '妥善保管每份收获。扩建增加 80 格容量，需要商队带回的建材。', category: 'services', cost: 680, wood: 20, stone: 15, frame: 9 },
  firetower: {footprint: 2,  name: '森林瞭望塔', description: '守望半径 4 格内的建筑，预防火灾，让居民安心入睡。', category: 'services', cost: 560, wood: 18, stone: 15, frame: 10, fireRadius: 4 },
  garden: {footprint: 1,  name: '花语小花园', description: '为小镇增添花香与散步的去处，每座花园改善环境和心情。', category: 'decoration', cost: 120, wood: 2, stone: 3, frame: 11, environment: 15 },
  oak: {footprint: 1,  name: '青荫橡树', description: '种下一片宽阔树荫，为街角和家门添绿。环境 +5，不需要工人。', category: 'decoration', cost: 60, wood: 0, stone: 0, frame: null, environment: 5 },
  cherry: {footprint: 1,  name: '春樱花树', description: '粉色花冠点亮河岸，留一条路给邻居赏花。环境 +6。', category: 'decoration', cost: 95, wood: 0, stone: 0, frame: null, environment: 6 },
  pine: {footprint: 1,  name: '常青松树', description: '四季常青的松树，适合矿区边缘和林间小径。环境 +4。', category: 'decoration', cost: 40, wood: 0, stone: 0, frame: null, environment: 4 },
  maple: {footprint: 1,  name: '晚霞红枫', description: '一树暖红，为住宅和田野添一抹秋色。环境 +6。', category: 'decoration', cost: 85, wood: 0, stone: 0, frame: null, environment: 6 },
  fountain: {footprint: 1,  name: '涟漪喷泉', description: '石雕喷泉与清澈水声，让街角有了小广场的气息。环境 +12，不替代水井。', category: 'decoration', cost: 320, wood: 0, stone: 12, frame: null, environment: 12 },
  gazebo: {footprint: 1,  name: '听风凉亭', description: '红瓦木亭，适合安放在花园与河畔空地。环境 +10。', category: 'decoration', cost: 260, wood: 12, stone: 4, frame: null, environment: 10 },
  bench: {footprint: 1,  name: '花畔长椅', description: '一张木长椅与两盆鲜花，给街边留个歇脚的地方。环境 +4。', category: 'decoration', cost: 65, wood: 3, stone: 1, frame: null, environment: 4 },
  flowerarch: {footprint: 1,  name: '蔷薇花拱门', description: '开满蔷薇的小拱门，布置在花园入口旁。环境 +8，占用装饰地块，不能铺路穿过。', category: 'decoration', cost: 180, wood: 6, stone: 2, frame: null, environment: 8 },
  flowerbox: {footprint: 1,  name: '临街花箱', description: '一排木花箱，把门前的窄边也种满花。环境 +5。', category: 'decoration', cost: 70, wood: 3, stone: 0, frame: null, environment: 5 },
  trellis: {footprint: 1,  name: '蔷薇花架', description: '让蔷薇顺着木架往上爬，适合贴着墙面布置。环境 +7。', category: 'decoration', cost: 110, wood: 5, stone: 1, frame: null, environment: 7 },
  archlights: {footprint: 1,  name: '串灯花门', description: '花门挂起一串暖灯，傍晚的街角会亮起来。环境 +9。', category: 'decoration', cost: 190, wood: 6, stone: 2, frame: null, environment: 9 },
  boardwalk: {footprint: 1,  name: '木栈道', description: '沿河铺一段木板路，雨天也好走。环境 +6。', category: 'decoration', cost: 130, wood: 8, stone: 2, frame: null, environment: 6 },
  railing: {footprint: 1,  name: '滨河栏杆', description: '一道矮木栏，把河岸收得整齐些。环境 +4。', category: 'decoration', cost: 55, wood: 4, stone: 0, frame: null, environment: 4 },
  parasol: {footprint: 1,  name: '遮阳伞座', description: '一顶条纹遮阳伞配桌椅，是午后最好的位置。环境 +8。', category: 'decoration', cost: 160, wood: 5, stone: 2, frame: null, environment: 8 },
  willow: {footprint: 1,  name: '垂柳', description: '枝条垂到水面上，适合种在河湾。环境 +7。', category: 'decoration', cost: 100, wood: 0, stone: 0, frame: null, environment: 7 },
  dock: {footprint: 1,  name: '浮动码头', description: '小木筏停靠的地方，给河边添一点生活气。环境 +8。', category: 'decoration', cost: 175, wood: 9, stone: 3, frame: null, environment: 8 },
  crates: {footprint: 1,  name: '木箱堆', description: '工坊门口的木箱，堆得稳当也是一景。环境 +4。', category: 'decoration', cost: 60, wood: 4, stone: 0, frame: null, environment: 4 },
  barrels: {footprint: 1,  name: '橡木酒桶', description: '两只旧木桶，摆在酒坊和市集旁正合适。环境 +4。', category: 'decoration', cost: 75, wood: 5, stone: 1, frame: null, environment: 4 },
  anvil: {footprint: 1,  name: '旧铁砧', description: '退役的铁砧与木墩，铁匠铺门口的老伙计。环境 +5。', category: 'decoration', cost: 90, wood: 2, stone: 3, frame: null, environment: 5 },
  signflags: {footprint: 1,  name: '彩旗招牌', description: '挂着彩旗的木招牌，让街口一眼就认得出。环境 +6。', category: 'decoration', cost: 105, wood: 5, stone: 1, frame: null, environment: 6 },
  cowbarn: {footprint: 2,  name: '青草牛舍', description: '先用 2 袋口粮照料小牛 150 秒，成年后每轮用 2 袋饲料产出 4 份鲜奶，自动收好。', category: 'production', cost: 520, wood: 16, stone: 8, frame: null, technology: 'husbandry', cycle: 34, input: { feed: 2 }, output: { milk: 4 }, workers: 1, autoCollect: true },
  dairy: {footprint: 2,  name: '山泉奶坊', description: '把 4 桶鲜奶做成 2 块奶酪，存放越久越值钱。', category: 'production', cost: 780, wood: 18, stone: 14, materials: { materials: 2 }, technology: 'husbandry', frame: null, cycle: 42, input: { milk: 4 }, output: { cheese: 2 }, workers: 2 },
  apiary: {footprint: 2,  name: '百花蜂场', description: '沿着花田放蜂箱，每轮自然采集 3 罐蜂蜜，不需要原料。', category: 'production', cost: 360, wood: 12, stone: 4, frame: null, technology: 'husbandry', cycle: 30, output: { honey: 3 }, workers: 1 },
  vineyard: {footprint: 2,  name: '南坡葡萄园', description: '向阳的坡地适合种葡萄，每轮自然结出 6 串。', category: 'production', cost: 460, wood: 14, stone: 6, frame: null, technology: 'viniculture', cycle: 38, output: { grape: 6 }, workers: 1 },
  winery: {footprint: 2,  name: '木桶酿酒坊', description: '把 6 串葡萄酿成 3 桶葡萄酒，也可以在「生产方向」里改酿啤酒（6 篮啤酒花 → 3 桶）。', category: 'production', cost: 860, wood: 20, stone: 16, materials: { materials: 3 }, technology: 'viniculture', frame: null, cycle: 56, input: { grape: 6 }, output: { wine: 3 }, workers: 2 },
  cellar: {footprint: 2,  name: '石阶酒窖', description: '把 3 桶葡萄酒再存成 2 瓶陈年佳酿，慢慢来的味道更好，也更值得上远方的订单。', category: 'production', cost: 980, wood: 16, stone: 26, materials: { materials: 4 }, technology: 'viniculture', frame: null, cycle: 70, input: { wine: 3 }, output: { vintage: 2 }, workers: 1 },
  ...
  canefield: {footprint: 2,  name: '河湾甘蔗田', description: '湿热的水边适合甘蔗，每轮收 6 捆，送往糖厂熬成砂糖。', category: 'production', cost: 400, wood: 12, stone: 6, frame: null, cycle: 34, output: { sugarcane: 6 } },
  hopsfield: {footprint: 2,  name: '棚架啤酒花园', description: '架起藤棚种啤酒花，每轮收 6 篮，交给酿酒坊酿成啤酒。', category: 'production', cost: 430, wood: 14, stone: 4, frame: null, technology: 'viniculture', cycle: 36, output: { hops: 6 } },
  hunterlodge: {footprint: 2,  name: '林间猎人屋', description: '猎人带回 2 份鲜肉和 2 张皮毛。肉送肉铺，皮毛卖给远方——猎场不会枯竭，只是收成没有作坊那么稳。', category: 'production', cost: 440, wood: 14, stone: 6, frame: null, cycle: 50, output: { pelt: 2, meat: 2 }, workers: 1, autoCollect: true },
  herbgarden: {footprint: 2,  name: '栅栏药草园', description: '一畦药草，每轮收 4 束。安心诊所备着药草时能照顾更多人，没有也不影响它本来的工作。', category: 'production', cost: 380, wood: 10, stone: 6, frame: null, cycle: 32, output: { herbs: 4 }, workers: 1, autoCollect: true },
  pigfarm: {footprint: 2,  name: '橡树猪舍', description: '用 2 袋饲料养一群猪，每轮出 4 份鲜肉，送到肉铺做成香肠。', category: 'production', cost: 520, wood: 16, stone: 8, frame: null, technology: 'husbandry', cycle: 44, input: { feed: 2 }, output: { meat: 4 }, workers: 1, autoCollect: true },
  butcher: {footprint: 2,  name: '街角肉铺', description: '把 3 份鲜肉做成 3 挂香肠。香肠要送到晚风酒馆才会被吃掉，否则只能卖出去。', category: 'production', cost: 680, wood: 18, stone: 14, materials: { materials: 2 }, frame: null, technology: 'husbandry', cycle: 44, input: { meat: 3 }, output: { sausages: 3 }, workers: 1, autoCollect: true },
  sugarmill: {footprint: 2,  name: '老石糖厂', description: '把 6 捆甘蔗熬成 3 份砂糖。糖是面包房的新配方，也是远方才见得着的好价钱。', category: 'production', cost: 640, wood: 16, stone: 14, frame: null, cycle: 40, input: { sugarcane: 6 }, output: { sugar: 3 }, workers: 1 },
  flowercart: {footprint: 1,  name: '花车', description: '花巷成型后立起的纪念花车，四季都装着刚剪下的花。环境 +6。', category: 'decoration', cost: 160, wood: 6, stone: 1, frame: null, environment: 6 },
  picniccorner: {footprint: 1,  name: '野餐角', description: '滨河步道连成一线后添的野餐桌，晴天总有人坐着。环境 +6。', category: 'decoration', cost: 160, wood: 7, stone: 1, frame: null, environment: 6 },
  harvestpile: {footprint: 1,  name: '丰收堆', description: '工坊广场成气候后堆起的南瓜与谷物，是小镇给手艺人的记功。环境 +6。', category: 'decoration', cost: 160, wood: 2, stone: 2, frame: null, environment: 6 },
  farm: {footprint: 1,  name: '田园农田', description: '免费选种，收获后自动续种。园艺成长解锁果蔬，土地越种越肥沃；小麦留仓，果蔬自动送农摊。', category: 'production', cost: 90, wood: 3, stone: 0, frame: null, cycle: 30, output: { wheat: 4 }, workers: 0 },
};
...
export const RESOURCES: Record<Resource, { name: string; icon: string; sellPrice: number }> = {
  ...
  // Meat is an ingredient for the butcher, not a staple ration: docs/04 §28 sends the catch
  // and the herd to the shop rather than to the table, and the town's larder stays fish and
  ...
  // Pelts come off the hunt and nothing works them into anything: they are the town's cash
...
export const TECHNOLOGIES: Record<TechnologyId, TechnologyDefinition> = {
  ...
  metallurgy: { name: '炉火与铁器', description: '将矿石变成金属与工具，也能立起城堡，接下更有价值的邻里委托。', branch: 'industry', icon: 'tools', level: 4, prestige: 6, coins: 480, items: { materials: 4 }, requires: ['mining'], unlocks: ['smelter', 'smithy', 'firestation', 'castle'] },
  husbandry: { name: '牧野的新朋友', description: '用麦穗养育小羊，把农田延伸成牧场。', branch: 'pastoral', icon: 'wool', level: 3, prestige: 4, coins: 240, items: { materials: 2 }, requires: [], unlocks: ['feedmill', 'pasture', 'fishpond', 'pigfarm', 'butcher'] },
...
export const PRODUCTION_SEQUENCE: BuildingKind[] = ['flowernursery','orchardhouse','jamkitchen','lumber', 'forester', 'quarry', 'farm', 'fishery', 'apiary', 'vineyard', 'canefield', 'hopsfield', 'mine', 'kiln', 'sawmill', 'windmill', 'feedmill', 'hunterlodge', 'herbgarden', 'pigfarm', 'butcher', 'chickencoop', 'fishpond', 'sugarmill', 'bakery', 'pasture', 'cowbarn', 'smelter', 'brickworks', 'dairy', 'weaver', 'winery', 'smithy', 'cellar', 'tailor'];
